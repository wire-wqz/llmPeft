...
SFT:已经会说话、会续写”的预训练大模型基础上，再用“目标任务的标准答案数据”教它按你想要的方式回答

数据集形式：问答

训练数据：
输入：问：介绍中南大学 回答：中南大学是985
标签：-100,-100,...,-100 ,回答：中南大学是985  通过-100屏蔽问题
...
def main():
    ...
    # 根据对话模板补齐 tokenizer 必要的特殊 token，并决定训练时“哪些位置不参与 loss 计算”
    #script_args.template_name = qwen
    prompt_template = get_conv_template(script_args.template_name)  
    # 如果 tokenizer 没有 eos token，就补一个 序列结束
    ...
    # 训练算 loss 时，padding 位置要不要忽略
    # ignore_pad_token_for_loss = true 则忽略
    IGNORE_INDEX = LabelSmoother.ignore_index if data_args.ignore_pad_token_for_loss else tokenizer.pad_token_id

    # 在SFT中不设置block_size，因为PT中为自回归任务，为了提高利用率采用先拼接再按照block_size切块
    # 而SFT 的样本通常天然就是“一条完整任务”，不能拼接按样本截断，最长训练长度直接设置为model_max_length
    # PT:  先拼接，再按 block_size 切
    # SFT: 先按单样本组织，再按 max_length 截断
    ...
    def preprocess_function(examples):
        """
        把多轮对话原始数据，整理成大模型 SFT 训练可直接使用的 input_ids / attention_mask / labels 
        Preprocessing the datasets.
        ...
        {
            "input_ids": [[
                101, 11, 12, 13, 14, 15, 21, 22, 23, 24, 2, 
                31, 32, 33, 41, 42, 43, 2                    
            ]],
            "attention_mask": [[
            ...
            "labels": [[
                -100, -100, -100, -100, -100, -100, 21, 22, 23, 24, 2, 
                -100, -100, -100, 41, 42, 43, 2                       
            ]]
        }
        
        # 解释：
        input_ids[0]
        101, 11, 12, 13, 14, 15 → system + 第一句 human + assistant 开头提示
        21, 22, 23, 24 → 第一轮 assistant 的回答：“你好，有什么可以帮你？”
        2 → 第一轮结束的 <eos>

        labels[0]
        100, -100, -100, -100, -100, -100 → system + 第一句 human + assistant 开头提示 不计算梯度（不用重复回答问题）
        21, 22, 23, 24, →  你好，有什么可以帮你？  学会回答
        2, → 第一轮结束的 <eos>
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="Train dataset tokenization"):
            # 数据通过preprocess_function函数处理，转化为SFT的格式
    ...
    if model_args.model_name_or_path:
        ...
        # Set RoPE scaling
        # 如果用户指定了 RoPE scaling，就修改模型配置 config，让模型支持比原来更长的上下文长度
        ...
        # 如果开启了 NEFTune，就在 fine-tuning 时给输入 embedding 加一点随机噪声
        ...
        # 当模型是 Mixtral，而且训练使用 DeepSpeed ZeRO-3 时，专门给 Mixtral 的 MOE 模块打一个兼容补丁。
        ...
        # 如果模型是 DeepSeek-V3，并且启用了 DeepSpeed ZeRO-3，就把它的 MoE 模块标记成 ZeRO-3 的叶子节点
