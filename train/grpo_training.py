...
def format_reward(completions, **kwargs):
    ...
    # 1. 定义正则表达式模式
    # r"..." : 原始字符串，避免转义字符问题
    # <think> : 匹配开始的 think 标签
    # .*?     : 非贪婪匹配任意字符
    # </think>: 匹配结束的 think 标签
    # <answer>: 紧接着匹配 answer 开始标签
    # .*?     : 非贪婪匹配答案内容
    # </answer>: 匹配答案结束标签
    # $       : 【关键】匹配字符串的结尾。意味着 </answer> 之后不能有任何其他字符（除了可能的换行，取决于 re 标志）  
    ...
    # 3. 执行匹配
    # 如果完全匹配成功 -> 1.0 分  如果格式错误（如缺少标签、顺序颠倒、后面有多余废话） -> 0.0 分
...
def grpo_train(
    ...
    with training_args.main_process_first(desc="Dataset preparation"):
        '''
        原始数据集转换为模型训练所需的对话格式 (Chat Format)，并准备好奖励计算所需的标准答案 (Ground Truth)
        '''
        # 加入系统提示词，用于创造不同输出
        dataset = dataset.map(
            lambda x: {
                'prompt': [
                    ...
                    {'role': 'user', 'content': x['question']}  # 这个 answer 不会直接输入给模型生成，而是传给 accuracy_reward 函数用于比对
                ],
                'answer': x['answer']
            },
    ...
    trainer = GRPOTrainer(
        ...
        reward_funcs=[            # 两个奖励函数 更准确地说是模型生成的每一个回答，这两个函数都会被执行
            accuracy_reward,
            format_reward
        ],
        args=training_args,
...
def main():
    ...
    # 训练控制参数封装到GRPOConfig->training_args中 自动执行
