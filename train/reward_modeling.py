...
class RewardDataCollatorWithPadding:
    ...
    把 reward model 训练数据整理成一个 batch，并且分别对 chosen 和 rejected 做 padding
...
class RewardTrainer(Trainer):
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        ...
        # Keep the compute_loss method 计算损失
        loss = -torch.nn.functional.logsigmoid(rewards_chosen - rewards_rejected).mean()
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        #     trust_remote_code=model_args.trust_remote_code,
        # )
        # 加载一个分类模型num_labels=1，本质上是一个评分模型，输出分数
        model = AutoModelForSequenceClassification.from_pretrained(
            model_args.model_name_or_path,
            config=config,
            torch_dtype=torch_dtype,
    ...
    def preprocess_reward_function(examples):
        ...
        将数据集转换为问题 + 回答的对形式，其中 input_ids_chosen 是首选的问题 + 回答，而 text_rejected 则是另一个答案。
        ...
        输出：
        new_examples = {
            "input_ids_chosen": [101, 11, 12, 13, 21, 22, 23, 24, 102],  # 问题回答对：你是一个有帮助的医疗助手，你好，请问有什么可以帮你？建议先休息、多喝水，若高烧持续不退请及时就医。
            "attention_mask": [1,   1,  1,  1,  1,  1,  1,  1,  1]
            "input_ids_rejected": [101, 11, 12, 13, 31, 32, 33, 102],    # 问题回答对：你是一个有帮助的医疗助手，你好，请问有什么可以帮你？随便吃点药就行，不用管。
